...
def score_out_of_focus(f: AstroFeatures) -> float:
    ...
    - sharpness ต่ำ
    - mean_intensity ต่ำ
    - background_median ต่ำ
    - background_mad ต่ำ
    - mean_hollowness สูง
    - FWHM / HFR สูง

    จุดสำคัญ:
    OOF และ Tracking มี FWHM ใกล้กันมาก
    จึงไม่ใช้ FWHM เป็นตัวตัดสินหลักเพียงอย่างเดียว

    เพิ่ม anti-tracking:
    - ถ้า sharpness สูง
    - intensity/background สูง
    จะลดคะแนน OOF
    """
    ...
    base = weighted_mean(
        [
            ...
            # OOF มี hollowness สูงกว่า Tracking ชัดเจน
            (
                score_high(
                    f.mean_hollowness,
                    0.005,
            ...
            # Streak ไม่ควรเป็นลักษณะหลักของ OOF
            (
                score_low(
    ...
    # - background สูง
    #
    # อย่าใช้ elongated_ratio เป็นตัวหักหลัก
    # เพราะข้อมูลล่าสุด OOF median (.126)
    # สูงกว่า Tracking (.084)
    # =========================================================

    tracking_confusion = weighted_mean(
        [
            (
                score_high(
                    f.sharpness,
    ...
    # Soft penalty
    anti_tracking_gate = 1.0 - 0.40 * tracking_confusion

    # =========================================================
    # 3) ANTI-SATELLITE
    #
    # ไม่ต้องแรงมาก เพราะ Satellite มี streak
    # ที่แยกออกจาก OOF ได้ค่อนข้างดี
    # =========================================================

    satellite_confusion = weighted_mean(
        [
            (
                score_high(
                    f.max_streak_length_ratio,
    ...
    anti_satellite_gate = 1.0 - 0.20 * satellite_confusion
...
def score_tracking_error(f: AstroFeatures) -> float:
    ...
    หลักฐานหลัก:
    - sharpness สูงกว่า OOF
    - mean_intensity สูง
    - background_median สูง
    - elongated stars
    - aspect ratio สูง
    - eccentricity สูง
    - FWHM ratio สูง
    ...
    base = weighted_mean(
        [
            ...
            # -------------------------------------------------
            #
            # Tracking median ~52,667
            # OOF median ~22,165
            #
            # ใช้เป็น feature สำคัญ
            (
                score_high(
                    f.sharpness,
                    30000.0,
            ...
            # -------------------------------------------------
            #
            # ไม่ให้ elongated ratio หนักเกินไป
            # เพราะ OOF ก็มี ratio สูงได้
            (
                score_high(
    ...
    # - hollowness สูง
    #
    # จุดนี้สำคัญที่สุดในการลด
    # OOF -> Tracking จำนวน 34/50
    # =========================================================

    oof_confusion = weighted_mean(
        [
            (
                score_low(
                    f.sharpness,
    ...
    # Soft penalty
    anti_oof_gate = 1.0 - 0.45 * oof_confusion

    # =========================================================
    # 3) ANTI-OVER-SATURATED
    #
    # OverSat -> Tracking = 28/50
    #
    # OverSat มี:
    # - bright_area_ratio สูง
    # - star_count สูง
    # - sharpness ต่ำมากในหลายภาพ
    # - eccentricity สูง
    # =========================================================

    oversat_confusion = weighted_mean(
        [
            (
                score_high(
                    f.bright_area_ratio,
    ...
    anti_oversat_gate = 1.0 - 0.35 * oversat_confusion

    # =========================================================
    # 4) ANTI-SATELLITE
    #
    # Tracking -> Satellite = 9/50
    #
    # Satellite มี streak ยาวมาก:
    #
    # Tracking median max_streak_length_ratio ~0.10
    # Satellite median ~0.707
    #
    # ดังนั้น feature นี้แยกได้ดี
    # =========================================================
    ...
    anti_satellite_gate = 1.0 - 0.35 * satellite_confusion
