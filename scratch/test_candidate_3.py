...
def candidate_te_3(f: AstroFeaturesV2) -> float:
    ...
    # Guard only if stars are round-ish discs (aspect < 1.50)
    # A true optical donut is round, whereas trails with dips/gaps have aspect >= 1.50
    if ((f.mean_hollowness > 0.08 and f.mean_aspect_ratio < 1.50) or (f.mean_star_area > 200.0 and f.mean_aspect_ratio < 1.60)) and f.elongated_angle_consistency < 0.70:
        return 0.05
...
print("--- Testing Candidate 3 (Hollowness donut check restricted to round discs < 1.50) ---")
evaluate_candidate(candidate_good_1, candidate_te_3)
