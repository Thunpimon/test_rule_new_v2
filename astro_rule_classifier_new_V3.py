...
#             elongated_count += 1
#             elongated_angles.append(elongated_axis_angle(contour))
...
def score_tracking_error(f: AstroFeatures) -> float:
    ...
    # เพิ่ม 2026 (ทดลอง — ยังไม่มีข้อมูลจริงยืนยัน threshold): gate จาก elongated_radial_alignment
    # แนวคิด: tracking error ทำให้ดาว "รีไปทิศเดียวกันทั้งภาพ" ไม่ว่าดาวอยู่ตรงไหน (ทิศทางไม่สัมพันธ์
    # กับตำแหน่ง) ต่างจาก coma aberration ที่ขอบเลนส์ ซึ่งทำให้ดาวรี "พุ่งออกจากศูนย์กลางภาพ" ตาม
    # ตำแหน่ง (ทิศทางสัมพันธ์กับตำแหน่งสูง) — ถ้า elongated_radial_alignment สูง (ดาวรีตาม radial
    # direction ชัดเจน) น่าจะเป็นเลนส์เพี้ยน ไม่ใช่ TE แท้ๆ จึงกดคะแนนลง
    # guard: ต้องมีดาวรีอย่างน้อย 3 ดวงขึ้นไปถึงจะเชื่อค่าเฉลี่ยนี้ได้ ไม่งั้นถือเป็นกลาง (ไม่กด/ไม่หนุน)
    if f.elongated_star_count >= 3:
        radial_gate = score_low(f.elongated_radial_alignment, 0.55, 0.80)
    else:
        radial_gate = 1.0
