...
async function preloadModel() {
    try {
        ...
        // กำหนด Options เพื่อเปิดใช้งานการ์ดจอผ่าน WebGL และมี WASM เป็นตัวสำรอง
        const options = { 
            executionProviders: ['webgl', 'wasm'] 
        };
        
        // ใส่ options เข้าไปเป็นพารามิเตอร์ตัวที่ 2
...
function buildManifestRows(completedItems) {
    const rows = [
        [
            ...
            'rule_score',
            'rule_label',
            ...CLASS_NAMES.map((className) => `confidence_${toColumnName(className)}`),
            'star_count',
            'sharpness',
            'saturated_ratio',
            'bright_area_ratio',
    ...
    completedItems.forEach((item) => {
        ...
        rows.push([
            item.file.name,
            top.className,
            top.probability.toFixed(6),
            top.modelProbability.toFixed(6),
            (top.ruleScore ?? RULE_NEUTRAL_SCORE).toFixed(6),
            top.ruleLabel ?? '',
            ...CLASS_NAMES.map((className) => (confidenceByClass[className] ?? 0).toFixed(6)),
            features?.starCount ?? '',
            features?.sharpness?.toFixed(3) ?? '',
            features?.saturatedRatio?.toFixed(6) ?? '',
            features?.brightAreaRatio?.toFixed(6) ?? '',
            features?.highlightRatio?.toFixed(6) ?? '',
            features?.elongatedStarRatio?.toFixed(6) ?? '',
            features?.longLineCount ?? '',
            latency.toFixed(0),
...
function renderPrediction({ item, ranked, latency, ruleAnalysis }) {
    ...
    card.dataset.confidence = top.probability.toString();
    ...
    card.querySelector('.top-confidence').textContent = `${formatPercent(top.probability)} final confidence`;
    card.querySelector('.rule-summary').textContent = `Model ${formatPercent(top.modelProbability)} | Rule ${formatPercent(top.ruleScore ?? RULE_NEUTRAL_SCORE)}`;
    card.querySelector('.rule-summary').title = top.ruleLabel ?? '';
...
function renderModalPrediction(item) {
    ...
    const { ranked, latency, ruleAnalysis } = item.prediction;
    const [top] = ranked;
    modalTopClass.textContent = top.className;
    modalTopConfidence.textContent = `${formatPercent(top.probability)} final | model ${formatPercent(top.modelProbability)} | rule ${formatPercent(top.ruleScore ?? RULE_NEUTRAL_SCORE)} | ${latency.toFixed(0)} ms`;
    const featureRows = ruleAnalysis?.features
        ? `
            <div class="rank-row rule-row">
                <span>Rule evidence</span>
                <strong>${top.ruleLabel}</strong>
                <small>Stars ${ruleAnalysis.features.starCount} | Sharp ${ruleAnalysis.features.sharpness.toFixed(1)} | Sat ${formatPercent(ruleAnalysis.features.saturatedRatio)} | Bright ${formatPercent(ruleAnalysis.features.brightAreaRatio)} | Lines ${ruleAnalysis.features.longLineCount}</small>
            </div>
        `
        : `
            <div class="rank-row rule-row">
